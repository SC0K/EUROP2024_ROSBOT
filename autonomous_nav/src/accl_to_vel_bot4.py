...
class AccelToCmdVel:
    def __init__(self):
        ...
        rospy.init_node('accel_to_cmd_vel4')
        ...
        # Subscribers
        rospy.Subscriber('/robot4/accel_x', Float32, self.accel_x_callback)
        rospy.Subscriber('/robot4/accel_y', Float32, self.accel_y_callback)
        rospy.Subscriber('/robot4/odom', Odometry, self.odom_callback)

        # Publisher
        self.cmd_vel_pub = rospy.Publisher('/robot4/cmd_vel', Twist, queue_size=10)
    ...
    def update_and_publish(self, event):
        ...
        rospy.loginfo(f"Forward velocity robot4: {self.forward_vel:.2f} m/s")
